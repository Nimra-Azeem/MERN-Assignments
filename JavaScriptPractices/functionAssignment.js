...
let createObject = (objectName) =>{
    users.push(objectName);
}
*/
//function to read an object 
const findElement = (users, Searchid) => {
    let foundUser = (user) => {
        return (user.id === Searchid ? true : false);
    }
    return users.find(foundUser);
}
let readObject = findElement(users, 3);
...
let UpdateObjMainFunc = (id, Object) => {
    let UpdateObject = findElement(users, id);
    for (i in UpdateObject){
        for(j in Object){
            if (UpdateObject[i]!= Object[j]) {
                UpdateObject[i] = Object[j];
            }
        }
    }
}
UpdateObjMainFunc(3,ObjecttoBeUpdated);
console.log(findElement(users, 3));
